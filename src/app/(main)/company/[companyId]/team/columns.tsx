...
import { UsersWithCompanySubAccountPermissionsSidebarOptions } from '@/lib/types'
import CustomModal from '@/components/global/custom-modal'

export const columns: ColumnDef<UsersWithCompanySubAccountPermissionsSidebarOptions>[] =
  [
    {
      accessorKey: 'id',
      header: '',
...

    {
      ...
      cell: ({ row }) => {
        const isCompanyOwner = row.getValue('role') === 'COMPANY_OWNER'
            ...
            <div className="flex flex-col items-start">
              <div className="flex flex-col gap-2">
                <Badge className="bg-slate-600 whitespace-nowrap">
                  Company - {row?.original?.Company?.name}
                </Badge>
              </div>
            </div>
          )
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <Badge
            className={clsx({
              'bg-emerald-500': role === 'COMPANY_OWNER',
              'bg-orange-400': role === 'COMPANY_ADMIN',
...
interface CellActionsProps {
  rowData: UsersWithCompanySubAccountPermissionsSidebarOptions
}
            ...
            onClick={() => {
              setOpen(
                ...
                >
                  <UserDetails
                    type="company"
                    id={rowData?.Company?.id || null}
                    subAccounts={rowData?.Company?.SubAccount}
                  />
                </CustomModal>,
                async () => {
          ...
          <AlertDialogAction
            ...
            onClick={async () => {
              ...
              toast({
                title: 'Deleted User',
                description:
                  'The user has been deleted from this company they no longer have access to the company',
              })
              setLoading(false)
              router.refresh()
            }}
