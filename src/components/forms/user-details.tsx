...
import {
  AuthUserWithCompanySidebarOptionsSubAccounts,
...
const UserDetails = ({ id, type, subAccounts, userData }: Props) => {
  ...
  const [authUserData, setAuthUserData] =
    useState<AuthUserWithCompanySidebarOptionsSubAccounts | null>(null);
  ...
  const onChangePermission = async (
    ...
    if (type === "company") {
      await saveActivityLogsNotification({
        companyId: authUserData?.Company?.id,
  ...
  const onSubmit = async (values: z.infer<typeof userDataSchema>) => {
    ...
    if (userData || data?.user) {
      ...
      authUserData?.Company?.SubAccount.filter((subacc) =>
      ...
      <CardContent>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="flex-1">
                  ...
                  <Select
                    disabled={field.value === "COMPANY_OWNER"}
                    onValueChange={(value) => {
                      if (
            ...
            {authUserData?.role === "COMPANY_OWNER" && (
              <div>
