...
const SendInvitation: React.FC<SendInvitationProps> = ({ companyId }) => {
  const { toast } = useToast()
  const userDataSchema = z.object({
    email: z.string().email(),
    role: z.enum(['COMPANY_ADMIN', 'SUBACCOUNT_USER', 'SUBACCOUNT_GUEST']),
  })
  ...
  const onSubmit = async (values: z.infer<typeof userDataSchema>) => {
    try {
      const res = await sendInvitation(values.role, values.email, companyId)
