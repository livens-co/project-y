...
export const saveActivityLogsNotification = async ({
  ...
  if (!foundCompanyId) {
    if (!subaccountId) {
      throw new Error(
        "You need to provide atleast an company Id or subaccount Id"
      );
    }
...
export const createTeamUser = async (companyId: string, user: User) => {
  if (user.role === "COMPANY_OWNER") return null;
...
export const upsertCompany = async (company: Company, price?: Plan) => {
...
export const getNotificationAndUser = async (companyId: string) => {
  try {
...
export const upsertSubAccount = async (subAccount: SubAccount) => {
  ...
  if (!companyOwner) return console.log("🔴Erorr could not create subaccount");
